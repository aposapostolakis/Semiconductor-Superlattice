function SLmbdis2(Ualas, Ugaas, dalas, dgaas)
    ...
    L = dgaas+dalas;  % Length of a SL period in nm
    ...
    for j = 1:NK
        ...
        % Periodic potential
        Uo = U2(x, Ualas_scaled, Ugaas_scaled, dalas_scaled, dgaas_scaled);
...
output_filename = 'miniband_properties_GaAsAlAsPRB.txt';
fid = fopen(output_filename, 'w'); % Open the file for writing

fprintf('GaAs-AlAs structure\n');
...
fprintf(fid, 'Length of a SL period: %.2f nm\n', L);
    ...
    print('dispersion_plot_GaAsAlasPRB.png', '-dpng') % Save as PNG image file
    pause;
end
